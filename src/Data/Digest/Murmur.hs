...
hashWithSalt :: Hashable a => Hash -> a -> Hash
hashWithSalt salt = flip runHashGen salt . hashGen
...
hashInt :: Int -> HashGen
hashInt =
  if finiteBitSize (undefined :: Int)  <= 32
...
instance Hashable Text where
    hashGen (Text arr off len) = foldl' combine (salt 0x0) (fmap hashGen xs)
      where
        xs = fmap (A.unsafeIndex arr) [off .. off + len - 1]